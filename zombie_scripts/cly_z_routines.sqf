...
//Add bandage
if (random 1<=CLY_zombiebandagechance or _zombietype in ["armored","slow armored"]) then {
	_action=[
...
/*
//Drop magazine
if (random 1<=CLY_zombiemagazinechance or _zombietype in ["armored","slow armored"]) then {
	_pos=getPosATL _zombie;
	_dir=vectorDir _zombie;
	_pos=[(_pos select 0)+(_dir select 0)*-1,(_pos select 1)+(_dir select 1)*-1,(_pos select 2)+0.5];
	_holder="WeaponHolder" createVehicle _pos;
	_holder setPosATL _pos;
	_holder setPosATL [_pos select 0,_pos select 1,(getPosATL _holder select 2)-(getPos _holder select 2)];
	...
	_holder setVehicleInit format ["this addMagazineCargo [""%1"",1]",_mag];
	processInitCommands;
};
*/